...
const AddCompanyModal: React.FC<AddCompanyModalProps> = ({ open, onClose }) => {
  ...
  const handleFormChange = (changedValues: any) => {
    if (changedValues.name !== undefined) {
      const suggestion = generateCompanyPrefixSuggestion(changedValues.name);
      const currentPrefix = form.getFieldValue('company_prefix');

      if (!currentPrefix || currentPrefix === lastSuggestedPrefix.current) {
        form.setFieldsValue({ company_prefix: suggestion });
        lastSuggestedPrefix.current = suggestion;
      }
    }

    if (changedValues.company_prefix !== undefined) {
      lastSuggestedPrefix.current = changedValues.company_prefix;
    }
  };
